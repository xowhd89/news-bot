...
# ==========================================
# 1. 환경 변수 및 API 세팅 (GitHub Secrets에서 불러옴)
# ==========================================
...
for cat in categories:
    ...
    if count > 0:
        # 이메일 HTML 포맷에 맞게 마크다운 대신 HTML 태그를 사용하도록 AI에게 지시
        prompt = f"""
...
중요: 마크다운 기호(*, # 등)를 절대 사용하지 말고, 반드시 HTML 태그(<p>, <ul>, <li>, <strong>, <h3>, <a href="..."> 등)를 사용하여 깔끔하게 구성하세요.
...
{cat['detail_rules']}
- 각 이슈는 <ul>과 <li> 태그를 사용하여 리스트 형태로 나열하고, 제목은 <strong>으로 강조하세요.
...
# ==========================================
# 4. 이메일 전송 로직 (Gmail SMTP)
# ==========================================
try:
    msg = MIMEMultipart('alternative')
    msg['Subject'] = f"[일간 브리핑] 종합 동향 보고서 ({today_str})"
    msg['From'] = GMAIL_ADDRESS
    msg['To'] = GMAIL_ADDRESS # 본인에게 보내기
